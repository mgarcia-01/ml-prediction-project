...
```{r sqlite, echo=FALSE,include=FALSE}
library(DBI)
...
plot(training$total_accel_arm,training$cvtd_timestamp)

```


### Correlations
Exploratory chart to display correlation strength between activities
```{r corchart, include=TRUE, echo=TRUE}
...
```{r modfit, tree, echo=TRUE}
library(caret)

inTrain <- createDataPartition(y=traindf$classe
                               ,p = 0.95
                               ,list = FALSE)
training <- traindf[inTrain,]
## remove row with NA's ##
...
```{r linear, include=TRUE, echo=TRUE}
modFit
#training
